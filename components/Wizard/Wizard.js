  ...
  constructor(id) {
    ...
    this.updateVisibleContents = () => {
      ...
      setTimeout(() => {
        ...
        + '.wizard-pf-contents:not(.hidden) form select')
          .first().focus(); // this does not account for disabled or read-only inputs
      }, 100);
    };
    ...
    this.tabGroupSelect = () => {
      $(`${self.modal} .wizard-pf-step>a`).click(function () {
        ...
        // in the tab group
        self.currentTab = $(`${self.modal} .list-group[data-tabgroup='${self.currentGroup}'] .list-group-item.active`)
          .data('tab');
        if (self.currentTab === undefined) {
          self.currentTab = $(`${self.modal} .list-group[data-tabgroup='${self.currentGroup}'] .list-group-item:first-child`)
          .data('tab');
    ...
    // success message).
    this.finish = () => {
      // if Back remains enabled during this step, then the Close button needs to be removed when the user clicks Back
      $(`${self.modal} .wizard-pf-back`).addClass('disabled');
