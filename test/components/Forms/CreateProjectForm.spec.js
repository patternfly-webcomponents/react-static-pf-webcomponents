...
describe('CreateProject Form test suite', () => {
  it('should set the project name when a value is passed to the component', () => {
    const handleSubmit = () => {};
    const value = { name: 'Test Project', description: 'We <3 Javascript' };

    const wrapper = mount(
      <CreateProjectForm handleSubmit={handleSubmit} value={value}/>
    );

    expect(wrapper.find('#exampleInputName').html())
      .toEqual('<input type="text" class="form-control" id="exampleInputName" value="Test Project">');
